<div class="bg-white dark:bg-background min-h-screen">
    <!-- Flash Messages -->
    @if(session()->has('owner_login_blocked'))
        @php($block = session('owner_login_blocked'))
        <dialog id="pending_modal" class="modal">
            <div class="modal-box bg-white">
                <div class="flex items-center justify-start mb-4">
                    ...
                    <h3 class="text-lg font-bold text-gray-900">{{ $block['title'] ?? 'Account Pending' }}</h3>
                </div>
                <p class="text-sm text-gray-700 py-4">{{ $block['message'] ?? '' }}</p>
                ...
                <div class="modal-action">
                    <form method="dialog">
                        <button class="btn bg-primary text-white hover:bg-primary/90">Close</button>
                    </form>
                </div>
            </div>
        </dialog>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const modal = document.getElementById('pending_modal');
                if (modal) {
                    modal.showModal();
